...
(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
		...
		_refresh: function(instant) {
			...
			if(instant && !$.isFunction(instant)) {
				...
				var displayTime = $(this.cssSelector.playlist + " ul").children().length ? this.options.playlistOptions.displayTime : 0;
					...
					var $this = $(this.cssSelector.playlist + " ul");
					$this.empty();
		...
		_createListItem: function(media) {
			...
			listItem += "<a href='javascript:;' data-id='" + media.id + "' class='" + this.options.playlistOptions.itemClass + "' tabindex='0'>" + media.title + (media.artist ? " <span class='jp-artist'>- " + media.artist + "</span>" : "") + "</a>";
		...
		play: function(index) {
			...
			$('.nmplayer-lrc').empty();
			$('.nmplayer-title').html(this.playlist[index].title + ' - ' + this.playlist[index].artist);	
...
if (0 < jQuery(".nmsingle-audio").length) {
	var q = "Microsoft Internet Explorer" == window.navigator.appName || "Netscape" == window.navigator.appName && /trident\/\d\.\d/i.test(window.navigator.userAgent) ? "flash, html" : "html, flash";
	jQuery(".nmsingle-audio").each(function() {
		var a = jQuery(this),
		...
		jQuery.ajax({
			...
			success: function(b) {
				if (200 == b.msg) {
					...
					a.find(".audio-name").append(b.song_title);
					a.find(".audio-author").append(b.song_author);
					var d = a.children(".audio-jplayer");
					d.jPlayer({
						ready: function() {
							d.jPlayer("setMedia", {
	...
	jQuery(".nmsingle-audio .play-button").click(function() {
	...
	jQuery(".nmsingle-audio .play-prosess").click(function(a) {
		var c = jQuery(this),
